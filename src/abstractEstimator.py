...
class BaseEstimator:
    def __init__(self, config, logger, model_type, version):
        ...
        self.target_encoding = self.config["parameter"]["common"]["target_encoding"]
        ...
        self.null_importance = self.config["mode"]["null_importance"]
    ...
    def kfold_fit(self, X: pd.DataFrame, y: np.array, cat_features: list = []) -> None:
        ...
        for fold_idx, (tr_idx, val_idx) in enumerate(kfold_generator):
            ...
            if len(self.target_encoding) > 0:
                raise Exception("Implement target encoding!!!")
            if self.model_type in ["linear", "nn"]:
                fill_values = X_train.mode().iloc[0].to_dict()
                X_train.fillna(fill_values, inplace=True)
                X_valid.fillna(fill_values, inplace=True)
                self.fill_values.append(fill_values)
            ...
            gc.collect()
        self.logger.info(
            f"\nmean valid score: {np.mean(valid_scores)}\tstd valid score: {np.std(valid_scores)}"
        )

    def kfold_predict(self, X_test: pd.DataFrame = None) -> None:
        self.pred_test = np.zeros((len(X_test)))
        if (
            self.kfold_method == "stratified"
            and self.stratified_column in X_test.columns
        ):
            X_test.drop(self.stratified_column, axis=1, inplace=True)
        if self.kfold_method == "group" and self.group_column in X_test.columns:
        ...
        for fold_idx in range(self.kfolds.n_splits):
            ...
            if len(self.target_encoding) > 0:
                raise Exception("Implement target encoding!!!")
            if self.model_type in ["linear", "nn"]:
                fill_values = self.fill_values[fold_idx]
    ...
    def _kfold_feature_importance(self, top_features: int = 60) -> pd.DataFrame:
        ...
        for i, model in enumerate(self.models):
            if self.model_type == "cb":
                ...
                raise Exception("Implement xgboost feature importance!!!")
            df_tmp = pd.DataFrame(
    ...
    def _select_columns(self, df: pd.DataFrame) -> None:
    ...
    def _drop_const_columns(self, df: pd.DataFrame) -> None:
        unused_columns = (
    ...
    def estimate(self, df: pd.DataFrame, is_train: bool) -> None:
        if is_train:
            ...
            self._drop_const_columns(self.X_train)
            print(self.X_train.shape)
            print(self.X_train.head(1))
            del df
            gc.collect()

            if self.null_importance:
                ...
                self.selected_features = self.selected_columns.copy()
